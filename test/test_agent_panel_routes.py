...
def _mounted(agent: str | None = CREW) -> web.Application:
    app = web.Application()
...
async def test_the_crew_is_not_taken_from_the_body(vetted):
    """A caller naming another crew must not reach that crew's webview."""
    c = await _client()
    resp = await c.post(
        "/api/agent-panel/publish",

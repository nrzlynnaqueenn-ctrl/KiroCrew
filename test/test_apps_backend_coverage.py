...
class TestDependencyInstall:
    ...
    def test_pip_runs_from_the_app_root_on_the_apps_own_requirements(
        ...
        directory — and -r points at the app's OWN requirements.txt, so a
        nested include (`-r requirements/base.txt`) resolves beside it
        (a relocated copy would break includes)."""
        (spawn_root / "server.py").write_text("x = 1\n")
        (spawn_root / "requirements.txt").write_bytes(b"requests\n")
    ...
    def test_an_editable_install_is_refused_loudly(
        self, spawn_root: Any, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """PYTHONPATH entries are not site dirs, so Python never processes
        the __editable__*.pth hook an editable install materialises as — the
        backend would spawn and die with ModuleNotFoundError. Provisioning
        must fail loudly instead, and nothing may be stamped or swapped
        live."""
        from kiro_crew.apps.interpreter import app_deps_dir
        ...
        assert "editable requirements" in log_text
    ...
    def test_tokenized_url_query_strings_are_stripped_from_pip_stderr(
        ...
        assert "<redacted-query>" in log_text

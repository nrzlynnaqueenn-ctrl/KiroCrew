...
def test_the_panel_lives_inside_the_crews_own_member_space():
    """Beside the DM binding and activity log, which are keyed the same way.

    This is what makes the panel belong to the CREW rather than to any one of its
    sessions -- and what makes whatever reaps a member space reap the panel too.
    """
    _publish()
    path = agent_panel.panel_path(CREW)
    assert path.parent == agent_panel.panel_dir(CREW)
    assert path.is_file()
...
def test_purge_removes_only_the_panel_record():
    """The member space around it belongs to the crew and holds other state."""
    _publish()
    sibling = agent_panel.panel_dir(CREW) / "keep-me.json"
    sibling.write_text("{}", encoding="utf-8")
    agent_panel.purge(CREW)
    assert not agent_panel.panel_path(CREW).exists()
    assert sibling.exists(), "purge must not take the crew's other state with it"


def test_purge_never_raises_for_a_hostile_slug():
    agent_panel.purge("a/b")
...
def test_the_mirrored_member_layout_matches_members():
    """The store derives the crew path itself; this is the anti-drift pin.

    If ``members`` ever moves its directory or widens its slug pattern, a
    mirrored copy that silently disagrees would put panels somewhere the rest of
    the crew's state is not.
    """
    from kiro_crew import members as members_mod

    assert agent_panel._MEMBERS_DIRNAME == members_mod.MEMBERS_DIR_NAME
    assert agent_panel._SLUG_RE.pattern == members_mod._SLUG_RE.pattern


def test_the_store_works_as_an_entry_point():
    ...
    probe = (
        "from kiro_crew import agent_panel;"
        " agent_panel.panel_dir('fleet-crew');"
    ...
    assert out.stdout.strip() == str(agent_panel.SCHEMA_VERSION)

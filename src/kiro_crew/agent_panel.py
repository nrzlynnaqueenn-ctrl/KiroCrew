...
def panel_dir(slug: str) -> Path:
    """The crew's own member space, where its panel lives.

    Validated then containment-checked, the same two steps
    ``members.member_dir`` takes: the pattern admits no ``/``, ``.`` or
    whitespace so a validated slug cannot traverse on its own, and the resolved
    path is re-checked so a symlinked root cannot land the panel outside the
    members tree either.
    """
    if not isinstance(slug, str) or not _SLUG_RE.match(slug):
        raise CrewSlugError(f"invalid crew slug {slug!r}: must match {_SLUG_RE.pattern}")
    root = (data_home() / _MEMBERS_DIRNAME).resolve()
...
def panel_path(slug: str) -> Path:
    """Absolute path to one crew's published panel record."""
    return panel_dir(slug) / _PANEL_FILE
...
def publish(
    ...
    *,
    ...
    record = {
        ...
        "template": template,
        "title": _clamp(title, _MAX_TITLE),
        "crew": _clamp(crew, _MAX_TITLE),
        "data": None,
        "published_at": _now_iso(),
    }
    blob = json.dumps(
        {**record, "data": _RAW(data_json)}, ensure_ascii=False, cls=_RawEncoder
    )
    if len(blob.encode("utf-8")) > _MAX_RECORD_BYTES:
        raise PanelError("record_too_large", "panel record exceeds its byte ceiling")

    dir_path = panel_dir(slug)
    with _locked(dir_path):
        atomic_write(dir_path / _PANEL_FILE, blob + "\n", mode=0o600)
...
def purge(slug: str) -> None:
    """Delete the crew's panel record. Best-effort, never raises.

    Only the record: the member space around it belongs to the crew and holds
    the DM binding and activity log too. A crew going away entirely takes its
    whole space -- and this file inside it -- with no help from here.
    """
    try:
        path = panel_path(slug)
    except ValueError:  # MemberSlugError
        return
    try:
        path.unlink()
    except OSError:

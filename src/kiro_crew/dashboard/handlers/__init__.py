...
def _prompt_dir_entry(path: Path, prompts_dir: Path, src: str) -> dict[str, Any] | None:
    ...
    * The stem must satisfy ``_plain_stem_ok``, the single predicate create, the
    ...
    return {
        "name": path.stem,
        "fullName": path.stem,
        # Read through the no-link gate, pinned inside the directory this entry
        # was gated against: opening by name would re-resolve the path, so an
        # entry swapped for a link between the lstat above and that open would
        # publish its TARGET's heading here. See _gated_sop_description.
        "description": _gated_sop_description(path, prompts_dir),

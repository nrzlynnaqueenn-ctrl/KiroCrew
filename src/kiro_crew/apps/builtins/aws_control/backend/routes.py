...
async def _handle_crews(request: web.Request) -> web.Response:
    ...
    try:
        inv = await asyncio.to_thread(
            crews_mod.list_crews, profile, region, account=account
        )
        ...
        return web.json_response(
            {"error": str(exc), "code": "account_mismatch"}, status=409
        )
    except AWSError as exc:
        return _aws_failed(exc)
    except RuntimeError as exc:
        return web.json_response(
            {"error": _safe_error(exc), "code": "aws_call_failed"}, status=502
        )

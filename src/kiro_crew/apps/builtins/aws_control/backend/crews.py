...
def _assert_account(profile: str, region: str, *, account: str) -> None:
    ...
    out = _checked(
        ["sts", "get-caller-identity", "--query", "Account", "--output", "text",
         "--region", region or engine.DEFAULT_REGION],
        profile,
...
def describe_crew(
    profile: str, region: str, *, account: str, crew: str
) -> Optional[RemoteCrew]:
    ...
    out = _checked(
        ["ecs", "describe-services", "--cluster", "smc", "--services", found.service,
         "--query", "services[0].[runningCount,desiredCount]", "--output", "json",
         "--region", found.region],
        profile,

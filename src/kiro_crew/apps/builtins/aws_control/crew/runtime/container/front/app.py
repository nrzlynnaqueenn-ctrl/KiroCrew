...
def _forward_body(payload: dict[str, Any], crew: str) -> tuple[dict[str, Any], str, bool]:
    ...
    """
    body: dict[str, Any] = {"stream": bool(payload.get("stream", False))}
    body["model"] = crew
    ...
    slot_id = raw_id if isinstance(raw_id, str) else ("" if raw_id is None else str(raw_id))
    return body, slot_id, body["stream"]

...
def run_restore(
    ...
    *,
    ...
    for full_key in sorted(listing):
        ...
        if not layout.is_config_key(rel):
            # Everything under data/ stays in the bucket: transcripts because a
            # task must not hold a conversation it did not serve, artifacts
            # because they are one customer's content too and the backend's
            # artifact store enumerates its whole directory with no per-customer
            # scope. Skipping is not deletion -- the sidecar only ever uploads,
            # so the object is still there for the on-demand fetch.
            result.skipped += 1
            continue

...
MIGRATION-MAP.md sends the S3 half of the old ``control/observe.py`` here, and
sends it deliberately unwritten. The reason is what the default deploy produces.

``--memory chatbot`` is the DEFAULT mode, and its whole claim is that nothing is
...
``--memory persistent`` writes transcripts under the crew's own prefix.

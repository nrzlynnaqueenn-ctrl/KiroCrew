...
# Any OTHER operator-facing script beside the driver. Today that set is empty:
# the image build scripts (build_image.sh, build_crew_image.sh) have no row in
# MIGRATION-MAP.md and so did not travel, and the driver dies naming the path it
# looked for. The loop stays because it is what covers them on arrival, without
# anyone having to remember this guard exists.

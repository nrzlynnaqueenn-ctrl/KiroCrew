...
# the ECR repo's `smc-base` tag when --base is absent.
#
...
smc_main() {
  ...
  MEMORY="chatbot"
  BASE_OPT=""            # maintainer base image, digest-pinned; resolved from ECR smc-base tag if empty
    ...
    case "$1" in
      ...
      --memory)  MEMORY="$2";  shift 2
                 case "$MEMORY" in chatbot|persistent) : ;;
                   *) die "--memory takes chatbot or persistent, not ${MEMORY}. The template
  ...
  dry_aws() {
    ...
    case "$svc $op" in
      ...
      "cloudformation wait") : ;;
      "ecr describe-images")
        # Resolve the base image digest from the 'smc-base' tag. SMC_FIX_BASE_DIGEST
  ...
  note "base      ${BASE_OPT:-<resolve from the ECR smc-base tag at step 4>}"
  ...
  if ! aws_ cloudformation deploy \
      ...
      --parameter-overrides "TranscriptRetentionDays=$RETENTION"; then
...
# otherwise it is resolved from the ECR repo's 'smc-base' tag (and we say which).
...
# The digest behind the moving 'smc-base' tag, or empty when it does not exist.
base_tag_digest() {
  if [ "$DRY_RUN" -eq 1 ]; then
...
build_base_image() {
  ...
  ecr_login
  "$builder" --arch "$CFN_ARCH" --repo "$REPO_URI" \
    || die "base image build failed (scripts/build_image.sh). Its output is above."
  ok "base image built and the 'smc-base' tag published"
}
...
resolve_base_ref() {
  if [ -n "${BASE_OPT:-}" ]; then
    ...
    if [ -z "$dg" ] || [ "$dg" = "None" ]; then
      ...
      dg="$(base_tag_digest)"
      [ -n "$dg" ] && [ "$dg" != "None" ] || die \
        "built the base image but the 'smc-base' tag still does not resolve.
   Pass --base <repo>@sha256:<hex> using the digest scripts/build_image.sh printed."
    fi
    BASE_REF="${REPO_URI}@${dg}"
    note "base image: $BASE_REF (resolved from the ECR 'smc-base' tag)"
  fi
...
#     passes TranscriptRetentionDays (NOTES/CHORUS R6: no default, blocks deploy).
#     REPORT to Templates track: confirm that is the only required base parameter.
#
...
#
#  9. PRE-EXISTING, unrelated to this track, in files this track owns: step 2 passes
#     `--parameter-overrides TranscriptRetentionDays=$RETENTION` to the BASE stack, but
#     templates/base.yaml declares no such parameter -- it declares LogRetentionDays.
#     `cloudformation deploy` rejects an override for a parameter the template does not
#     declare, so a real `--from <=2` base deploy would fail at CreateChangeSet. Not
#     touched here because it is outside the bundle-in-image change and the intended
#     fix (rename the base param, or pass LogRetentionDays, and decide transcript-
#     retention enforcement per base.yaml's own note) is a separate decision. REPORTED.
# ===========================================================================

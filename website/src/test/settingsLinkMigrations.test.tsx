...
import { capabilitiesVars, deployVars } from '../components/destinationVars'
...
describe('Settings-link migrations', () => {
  ...
  it('the AWS Control destination is spelled by the Artifacts rail and Artifact Deploy labels', () => {
    const expected = `${i18nT('nav.artifacts')} → ${i18nT('pages.artifactsPage.artifact_deploy')}`
    expect(i18nT('apps.awsControl.page.empty_body', deployVars())).toContain(expected)
    expect(i18nT('apps.awsControl.page.add_accounts_unsupported', deployVars())).toContain(expected)
    expect(i18nT('apps.awsControl.page.empty_body', { artifacts: 'X', deploy: 'Y' })).toContain('X → Y')
  })
})

 ...
 *  the dashboard something lives ("… under {{artifacts}} → {{deploy}}").
 *
...
/** `{{artifacts}} → {{deploy}}`: the Artifacts rail entry and its Artifact Deploy entry (the /deploy page). */
export function deployVars(): { artifacts: string; deploy: string } {
  return {
    artifacts: i18nT('nav.artifacts'),
    deploy: i18nT('pages.artifactsPage.artifact_deploy'),
  }
}

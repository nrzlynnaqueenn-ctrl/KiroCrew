 ...
 *  every consumer, instead of each call site remembering to. A modified or
 *  non-primary click (Cmd/Ctrl/Shift/Alt, middle button) opens the href in a
 *  new tab and unmounts nothing, so it is left to the browser and never asked.
 *
...
import { useIsCurrentUrl, useMayLeaveForNavigation } from './NavigationLeaveGuard'
...
export function SettingsLink({
  ...
  const onClick = (e: MouseEvent<HTMLAnchorElement>) => {
    ...
    // Navigating to where we already are unmounts nothing; asking would pop a
    // discard-confirm the user never earned.
    if (!isCurrentUrl(to) && !mayLeave()) {
      e.preventDefault()
      return
    }
    onPlainClick?.()

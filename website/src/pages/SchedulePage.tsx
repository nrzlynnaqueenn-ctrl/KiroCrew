    ...
    <div className="flex flex-col gap-1.5">
      {pending && (
        <div className="flex flex-col gap-2 px-3 py-2.5 rounded-lg bg-warn-subtle text-warn-fg" role="note">
          ...
          {source.isError && (
            <div className="text-danger text-[12.5px]" role="alert">
              {i18nT('pages.schedulePage.secrets_pending_source_failed')}
            </div>
          )}
          {source.data && source.data.truncated && (
            <div className="text-danger text-[12.5px]" role="alert">
              {i18nT('pages.schedulePage.secrets_pending_source_truncated')}
            </div>
          )}
          {source.data && !source.data.truncated && !source.data.reviewable && (
            <div className="text-danger text-[12.5px]" role="alert">
              {i18nT('pages.schedulePage.secrets_pending_source_unreviewable')}
            </div>
          )}

...
/** Suffix of the caveat convention: ``<field>_note`` qualifies ``<field>``. */
...
function summarize(meta: PanelMeta | null, slug: string): DockedSummary {
  ...
  const isAttachedNote = (k: string) =>
    k.length > NOTE_SUFFIX.length &&
    k.endsWith(NOTE_SUFFIX) &&
    Object.prototype.hasOwnProperty.call(data, k.slice(0, -NOTE_SUFFIX.length))
...
export default function CrewWebview({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/members/${encodeURIComponent(slug)}/panel`, { credentials: 'same-origin' })
      .then(r => (r.ok ? r.json() : Promise.reject(new Error(`http_${r.status}`))))
      .then((body: { panel: PanelMeta | null; html: string | null }) => {
        if (!alive) return
        if (!body.html) {
          setState('empty')
          return
  ...
  if (state === 'error') {
    return (
      <div className="text-[11px] text-muted mb-4" role="alert" data-testid="crew-webview-error">
        {i18nT('pages.membersPage.webview_error')}
      </div>
    )
  }
  ...
  return (
    <div
      ...
      aria-label={expanded ? collapseLabel : undefined}
    >
      {!expanded && (
        /*
        ...
        >
          ...
          >
            ...
            {ago && <span className="ml-auto font-mono shrink-0">{ago}</span>}
          </div>
          ...
          <div className="px-3 py-2.5">
            ...
            {summary.lead && (
              <div className="mt-2.5" data-testid="crew-webview-lead">
                <div className="text-[9px] font-mono uppercase tracking-[0.09em] text-muted mb-1">
                  {summary.lead.label}
                </div>
              ...
              <dl className="mt-2.5 pt-2 border-t border-border space-y-1">
                {summary.stats.map(s => (
                  <div key={s.key} className="flex items-baseline gap-2">
                    <dt className="text-[9px] font-mono uppercase tracking-[0.09em] text-muted">
                      {s.label}
                    </dt>
      ...
      {everExpanded && (
        <div className={expanded ? 'flex flex-col flex-1 min-h-0' : 'hidden'}>
          ...
          <div className="relative border border-border border-t-0 rounded-b-lg overflow-hidden bg-card flex-1 min-h-0">
            {failed && (
              <div className="absolute top-0 left-0 right-0 z-10 px-3 py-2 flex items-center gap-2 text-[11px] text-text bg-bg-elevated/95 border-b border-border">
                <span role="status">{i18nT('pages.membersPage.webview_error')}</span>
                <Btn disabled={pending} onClick={retry}>
